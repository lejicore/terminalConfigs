...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    ;;     (lsp-pyright-multi-root nil) ;; disable multi-root
    ;;     :hook
    ;;     (python-ts-mode . ox/python-lsp-start)
    ;;     :config
    ;;     (defun ox/python-lsp-start ()
    ;;         (require 'lsp-pyright)
    ;;         (lsp-deferred))) 

    (defun ox/python-configure-flycheck-chain ()
        "Run Pyright's Flycheck checker after LSP diagnostics in Python buffers."
        (when (and (derived-mode-p 'python-ts-mode)
                  (bound-and-true-p flycheck-mode)
                  (memq 'lsp flycheck-checkers)
                  (memq 'python-pyright flycheck-checkers))
            (flycheck-add-next-checker 'lsp 'python-pyright)))

    (add-hook 'lsp-configure-hook #'ox/python-configure-flycheck-chain t)
