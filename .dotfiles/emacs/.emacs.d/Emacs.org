...
(defun leji/acp-mobile-keep-awake (duration unit)
  "Keep this Mac reachable for ACP mobile for DURATION in UNIT.
UNIT must be either `minutes' or `hours'.  When another keep-awake period is
already active, offer to replace it."
  (interactive
   (progn
     (unless (eq system-type 'darwin)
       (user-error "ACP mobile keep-awake is available only on macOS"))
     (list
      (read-number "Keep Mac awake for: " 90)
      (intern
       (completing-read
        "Unit: " '("minutes" "hours") nil t nil nil "minutes")))))
  (unless (eq system-type 'darwin)
    (user-error "ACP mobile keep-awake is available only on macOS"))
  (unless (> duration 0)
  ...
  (let* ((multiplier
    ...
    (when (process-live-p leji/acp-mobile-caffeinate-process)
      (unless (yes-or-no-p "Replace the active ACP mobile keep-awake period? ")
        (user-error "Existing keep-awake period left unchanged"))
      (delete-process leji/acp-mobile-caffeinate-process))
...
(defun leji/acp-mobile-stop-keeping-awake ()
  ...
  (unless (eq system-type 'darwin)
    (user-error "ACP mobile keep-awake is available only on macOS"))
  (if (process-live-p leji/acp-mobile-caffeinate-process)
      (progn
        (delete-process leji/acp-mobile-caffeinate-process)
        (setq leji/acp-mobile-caffeinate-process nil)
