...
* macOS Emacs daemon must launch from Emacs.app

On macOS with AeroSpace, do NOT start the interactive daemon with:

#+begin_src sh
emacs --daemon
#+end_src

That launches the Nix store binary directly and macOS/AeroSpace sees Emacs as:

#+begin_example
NULL-APP-BUNDLE-ID
#+end_example

As a result, AeroSpace does not recognize Corfu child frames as Emacs popups.
When Corfu closes its floating child frame, AeroSpace may recalculate focus and
move keyboard focus to another tiled window.

Start the daemon through the executable inside the app bundle instead:

#+begin_src sh
"/Applications/Nix Apps/Emacs.app/Contents/MacOS/Emacs" --daemon
#+end_src

Then normal usage stays unchanged:

#+begin_src sh
emacsclient -r
#+end_src

Expected AeroSpace identity:
...
aerospace list-windows --monitor focused \
  --format '%{window-id}|%{app-pid}|%{app-bundle-id}|%{app-name}|%{app-exec-path}|%{window-title}|%{window-layout}'
#+end_src

Root cause found 2026-08-12:
Nix Emacs daemon launched via bin/emacs lost macOS bundle identity.
Launching Emacs.app/Contents/MacOS/Emacs preserves org.gnu.Emacs and fixes
Corfu/AeroSpace focus drifting.
* Things to test
...
(defcustom ox/config-repository-root
  (expand-file-name (or (getenv "TERMINAL_CONFIGS_ROOT") "~/terminalConfigs"))
  "Root of the terminalConfigs source checkout.
Set TERMINAL_CONFIGS_ROOT when the checkout lives elsewhere."
  :type 'directory
  :group 'environment)
(defvar ox/emacs-source-directory
  (expand-file-name ".dotfiles/emacs/.emacs.d/" ox/config-repository-root)
  "Directory containing the authoritative Emacs implementation modules.")
...
(use-package doom-modeline
  ...
  ;; The backend-neutral terminal module installs its count in
  ;; `mode-line-misc-info' after persp-mode loads.
  )
#+end_src
...
(use-package persp-mode
    ...
    :config
    ;; Workspace semantics, terminal backends, and editing-buffer navigation
    ;; are separate modules rooted in the source checkout, not in the runtime
    ;; `user-emacs-directory'.
    (dolist (module '("ox-workspace" "ox-ghostel-ssh" "ox-terminal"
                         "ox-buffer-navigation" "ox-window-zoom"))
        (load (expand-file-name module ox/emacs-source-directory) nil 'nomessage))

    (defun ox/persp-ephemeral-buffer-p (buffer)
        "Return non-nil when BUFFER should not be persisted by persp-mode."
        (with-current-buffer buffer
            (derived-mode-p 'agent-shell-mode)))

    (with-eval-after-load 'persp-mode
        (add-to-list 'persp-filter-save-buffers-functions
            #'ox/persp-ephemeral-buffer-p))

    (defun ox/persp-org-agenda-buffer-p (buffer)
        "Return non-nil when BUFFER visits one of my Org agenda source files."
        (let ((file (buffer-file-name buffer)))
            (and file
                (member (expand-file-name file)
                    (mapcar #'expand-file-name my-org-files)))))

    (with-eval-after-load 'persp-mode
        (add-to-list 'persp-filter-save-buffers-functions
            #'ox/persp-org-agenda-buffer-p))

    (defun consult-persp-buffer ()
        "Switch to a buffer within the current perspective using consult."
        (interactive)
        (let* ((persp-buffers
            (cl-loop for buffer in (persp-buffer-list-restricted)
    ...
    (defun my-persp-buffer-filter (buf)
        "Filter internal buffers, while retaining manager-owned terminals."
        (let ((buf-name (buffer-name buf)))
	    (and (not (buffer-local-value 'ox-terminal-managed-p buf))
                (string-prefix-p "*" buf-name)
                (not (string-prefix-p "*vterm" buf-name)))))
    ...
    (global-set-key (kbd "C-c v") #'ox-terminal-switch-to-last)
    (global-set-key (kbd my-switch-to-next-persp-vterm-key) #'ox-terminal-cycle)
    (global-set-key (kbd my-switch-to-prev-persp-vterm-key) #'ox-terminal-cycle-previous)
...
(defun my-switch-to-project ()
  ...
  (let* ((projects (append (mapcar #'identity (project-known-project-roots)) '("Add Project...")))
    ...
    (if (string-equal project "Add Project...")
      ...
      (ox-workspace-switch-to-project project)))
#+end_src
...
(use-package multi-vterm
    ...
    (ox/leader-keys
        ...
        "sv" '(ox-terminal-create :which-key "new managed terminal")
        "so" '(multi-vterm-dedicated-toggle :which-key "toggle multi-vterm")
        "sp" '(ox-terminal-cycle-previous :which-key "previous managed terminal")
        "sn" '(ox-terminal-cycle :which-key "next managed terminal")
...
(evil-define-key '(visual insert normal) eshell-mode-map (kbd "C-6") #'ox-buffer-navigation-switch-to-editing-buffer)
(evil-define-key '(visual insert normal) eshell-mode-map (kbd "C-\\ l") #'ox-buffer-navigation-switch-to-editing-buffer)
...
#+end_src
** ghostel
Ghostel keeps its native =xterm-kitty= capabilities locally.  A small smart
=ssh= shim is added only to the environment of Ghostel-created shells: it asks
OpenSSH for the effective destination, reuses one multiplexed connection,
probes or installs user-local terminfo, verifies it, and falls back only when
bootstrap is impossible.  Kitty's SSH helper inspired the behavior; no Kitty
implementation source is copied.  Set =ox-ghostel-ssh-enable= to nil for the
kill switch, or =ox-ghostel-ssh-debug= to t for sanitized diagnostics.
#+begin_src emacs-lisp
(use-package ghostel
  :straight t
  :custom
  (ghostel-term "xterm-kitty")
  :config
  (add-to-list 'ghostel-tramp-shells '("rpc" login-shell))
  :bind (:map project-prefix-map
              ("m" . ghostel-project)
              ("M" . ghostel-project-list-buffers)))
(defun ox/ghostel-send-backspace ()
  "Send backspace through Ghostel's documented key API."
  (interactive)
...
(use-package evil-ghostel
  ...
  :config
  (evil-define-key '(visual insert normal)
    evil-ghostel-mode-map (kbd "C-h") #'ox/ghostel-send-backspace))
#+end_src
* Archived vterm-only navigation reference
...
#+begin_src emacs-lisp
(defun my/crunner ()
  "Make and run a C program in a managed terminal based on Makefile recipes.
because compile mode is too slow"
  ...
  (if (or
	...
	(let ((target (concat "make && time " "./" (file-name-nondirectory (directory-file-name (file-name-directory buffer-file-name))) "\n"))
	      (terminal (or (ox-terminal-switch-to-last)
	                    (ox-terminal-create))))
	  (ox-terminal-send-string target terminal)))
    (print "Not in c-mode")))
(ox/leader-keys
  "cv" '(my/crunner :which-key "Run C code in terminal"))
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    (use-package lsp-pyright
        ...
        :hook
        (python-ts-mode . ox/python-lsp-pyright))

    (defun ox/python-lsp-pyright ()
        "Start Pyright through lsp-mode for Python."
        (require 'lsp-pyright)
        (lsp-deferred))
...
(defun ox/indent-bars-guard-stale-depth-timer (orig-fn buffer)
  "Ignore an indent-bars depth timer whose buffer state was torn down."
  (when (buffer-live-p buffer)
    (with-current-buffer buffer
      ;; The timer is one-shot; don't leave a stale timer object behind.
      (unless (and indent-bars-mode
                   (numberp indent-bars--offset))
...
(with-eval-after-load 'indent-bars
  (advice-add #'indent-bars--update-current-depth-highlight-in-buffer
              :around #'ox/indent-bars-guard-stale-depth-timer))
#+end_src
* Dash Docs
...
(defun ox/ledeb-vterm ()
  "Open an explicitly vterm-backed managed terminal on ledeb."
  (interactive)
  (let ((default-directory "/sshx:ledeb:~"))
    (ox-terminal-create 'vterm default-directory)))
...
(ox/leader-keys
    ...
    "wz" '(ox-window-zoom-toggle :which-key "toggle window zoom")
