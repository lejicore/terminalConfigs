...
client.connect_signal("request::default_keybindings", function()
	awful.keyboard.append_client_keybindings({
		awful.key({ modkey }, "Down", function()
			inc_dec_gap(-1)
		end, { description = "Decrease gap", group = "benji" }),
		awful.key({ modkey }, "Up", function()
			inc_dec_gap(1)
		end, { description = "Increase gap", group = "benji" }),
		awful.key({ modkey }, "g", function()
			toggle_gap()
		end, { description = "toggle gap", group = "benji" }),
		awful.key({ modkey, "Shift" }, "p", function(c)
			if awful.rules.match_any(c, virtual_fullscreen) then
				...
				naughty.notification({ title = "Claimed fullscreen ?", message = tostring(c.virtual_fullscreen) })
			end
