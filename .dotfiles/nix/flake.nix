{
  ...
  outputs =
    ...
    in
    {
      checks.${darwin-system} = {
        php-treesit-compatibility = darwin-pkgs.runCommand "php-treesit-compatibility" { } ''
        ...
        emacs-daemon-startup-safety = darwin-pkgs.runCommand "emacs-daemon-startup-safety" { } ''
            ${darwin-pkgs.emacsLejiWithPackages}/bin/emacs --batch --eval '
              (progn
                ...
                (with-temp-buffer
                  ...
                  "${../emacs/.emacs.d/Emacs.org}")
                (goto-char (point-min))
                (search-forward "(defun ox/straight-update-after-startup")
                (goto-char (match-beginning 0))
                (eval (read (current-buffer))))
              (let ((update-called nil))
                (cl-letf (((symbol-function (quote daemonp))
                           (lambda () t))
                          ((symbol-function (quote ox/straight-update-maybe))
                           (lambda () (setq update-called t))))
                  (ox/straight-update-after-startup))
                (when update-called
                  (error "Daemon startup attempted an interactive update prompt"))
                (cl-letf (((symbol-function (quote daemonp))
                           (lambda () nil))
                          ((symbol-function (quote ox/straight-update-maybe))
                           (lambda () (setq update-called t))))
                  (ox/straight-update-after-startup))
                (unless update-called
                  (error "Interactive startup no longer offers updates"))))'
          touch "$out"
        '';
      };
